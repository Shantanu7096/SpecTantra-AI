...
# Helper functions to persist settings without editing code
def load_config():
...
def generate_mjpeg_stream():
    while True:
        ...
        if frame is None:
            ...
            cv2.putText(blank, "AWAITING CAMERA STREAM...", (150, 240),
...
def calibrate():
    ...
    return jsonify({"status": "error", "message": "Calibration failed. Check camera feed."})
...
def save_test():
    ...
    try:
        ...
        target_csv = os.path.join(BASE_DIR, f"soil_database_backup_{timestamp_str}.csv")
...
def ai_chat():
    ...
    if ai_client and GEMINI_API_KEY not in ["YOUR_GEMINI_API_KEY_HERE", None]:
        try:
            system_prompt = (
                f"You are SpecTantra AI, an agricultural expert assisting an Indian farmer.\n"
                f"Live Soil Analysis Context:\n"
                f"- Nitrogen: {m['nitrogen']}\n"
                f"- Phosphorus: {m['phosphorus']}\n"
                f"- Potassium: {m['potassium']}\n"
                f"- pH: {m['ph']} ({m['ph_class']})\n"
                
                f"- Soil Quality Score: {m['score']}%\n\n"
                f"Farmer Question: '{user_query}'\n\n"
                f"INSTRUCTIONS:\n"
                f"1. Answer the farmer's question directly and comprehensively.\n"
                f"2. If asked about soil/crops, use the live data above. If asked general farming questions (fertilizers, brands, pests, weather), give expert agricultural advice.\n"
                f"3. Keep response concise (2 to 4 sentences).\n"
                f"4. Respond strictly in {target_lang}."
            )
    ...
    if any(k in q_lower for k in ["brand", "company", "which fertilizer", "best fertilizer", "ब्रांड", "कंपनी"]):
        ans_en = "Top trusted Indian fertilizer brands include IFFCO, Mahadhan, Coromandel, Kribhco, and Chambal Fertilizers. Choose NPK 19:19:19 or Urea based on crop requirements."
        ans_hi = "भारत में सबसे भरोसेमंद खाद ब्रांड इफ्को (IFFCO), महाधन (Mahadhan), कोरोमंडल (Coromandel) और कृभको (Kribhco) हैं।"
        ans_mr = "भारतातील प्रमुख खत ब्रँड इफको (IFFCO), महाधन (Mahadhan), कोरोमंडल आणि कृभको आहेत."
    elif any(k in q_lower for k in ["rice", "paddy", "चावल", "धान", "तांदूळ"]):
        ans_en = f"Rice grows well in this soil. Your soil pH of {m['ph']} and optimal NPK levels provide suitable conditions."
        ans_hi = f"हाँ, इस मिट्टी में धान की खेती बहुत अच्छी होगी। pH {m['ph']} अनुकूल है।"
        ans_mr = f"होय, या मातीत भात शेती उत्तम होईल. pH {m['ph']} योग्य आहे."
    elif any(k in q_lower for k in ["wheat", "गेहूं", "गहू"]):
        ans_en = f"Wheat is highly suitable for this soil with pH {m['ph']} and balanced nutrients."
        ans_hi = f"गेहूं की खेती के लिए यह मिट्टी बहुत अच्छी है।"
        ans_mr = f"गहू पिकासाठी ही माती उत्तम आहे."
    else:
        ans_en = f"For query '{user_query}': Soil pH is {m['ph']} ({m['ph_class']}). Advice: {m['recommendation']}"
...
# Endpoint to download the entire CSV file to Mobile
@app.route('/download/csv')
def download_csv():
    if os.path.exists(CSV_FILE):
        return send_file(CSV_FILE, as_attachment=True, download_name="soil_database.csv")
    return jsonify({"status": "error", "message": "No CSV file found yet."}), 444

# Endpoint to view or download saved test images
...
<head>
    ...
    <style>
        ...
        .video-container { position: relative; display: inline-block; width: 100%; cursor: crosshair; }
        .video-container img { width: 100%; border-radius: 8px; border: 2px solid #00d2ff; }
...
<body class="p-3">
    <div class="container-fluid">
        <!-- TOP NAV BAR WITH MOBILE CAMERA TOGGLE -->
<div class="d-flex justify-content-between align-items-center pb-3 mb-3 border-bottom border-secondary">
    <h3 class="m-0 text-info fw-bold">🔬 SpecTantra AI <span class="fs-6 text-light fw-normal">| Mobile System</span></h3>
    <div class="d-flex gap-2 align-items-center">
        <!-- Quick Camera Selector for Mobile / Laptop -->
        <select id="camSelect" class="form-select form-select-sm bg-dark text-light border-secondary" style="width: auto;" onchange="handleCamSelectChange(this.value)">
            <option value="0">Camera 0 (Front/Laptop)</option>
            <option value="1">Camera 1 (Rear/Back)</option>
            <option value="2">Camera 2 (External)</option>
            <option value="custom">IP Stream URL...</option>
        </select>
        <input type="text" id="camIpInput" class="form-control form-control-sm bg-dark text-light border-secondary d-none" placeholder="http://192.168.x.x:8080/video" style="width: 200px;">
        <button onclick="updateCameraIp()" class="btn btn-sm btn-primary">Switch Camera</button>
    </div>
</div>

        <div class="row g-3">
            <!-- LIVE VIDEO & SPECTRAL GRAPH -->
            <div class="col-lg-7">
                <div class="card p-3">
    ...
    <canvas id="frameCanvas" class="d-none"></canvas>
            ...
            <div class="col-lg-5">
                ...
                <div class="card p-3">
                    ...
                    <div class="p-3 bg-dark rounded border border-secondary" style="min-height: 90px;">
...
window.addEventListener('DOMContentLoaded', startWebcam);
        ...
        function sendAiQuery(queryText) {
            let text = queryText || document.getElementById('aiQueryInput').value;
        ...
        function startVoiceRecognition() {
            ...
            recognition.onstart = function() {
                document.getElementById('aiQueryInput').placeholder = "Listening...";
            };
        ...
        function speakText(text, lang) {
            ...
            function setVoice() {
                ...
                let langPrefix = lang.split('-')[0].toLowerCase();
        ...
        function shareWhatsApp() {
            let txt = `SpecTantra Soil Report: N=${currentAnalysis.nitrogen}, P=${currentAnalysis.phosphorus}, K=${currentAnalysis.potassium}, pH=${currentAnalysis.ph}. Health Index: ${currentAnalysis.score}%. Advice: ${currentAnalysis.recommendation}`;
    ...
    if (val === 'custom') {
        ...
        inputEl.value = val;
        updateCameraIp(); // Auto switch instantly on selecting Camera 0 or 1
    }
}
